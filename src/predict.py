...
Usage:
    ...
    python -m src.predict --input texts.txt

    # Use hybrid router (ML first, escalate to LLM if uncertain)
    python -m src.predict --mode hybrid --input texts.txt

    # ML-only (no API calls)
    python -m src.predict --mode ml --input texts.txt
"""
...
def predict_llm(texts: list[str], cfg: dict) -> list[dict]:
    ...
    from src.llm_classifier import HierarchicalLLMClassifier
...
def predict_ml(texts: list[str], cfg: dict) -> list[dict]:
    ...
    model_path = ROOT / "data" / "processed" / "ml_baseline.pkl"
    if not model_path.exists():
        print(f"Error: ML model not found at {model_path}. Run src.ml_baseline first.", file=sys.stderr)
        sys.exit(1)
