"""
Legacy research mode pipeline — reads pre-computed ML and (optionally) LLM prediction
parquets, computes hybrid routing, and produces a wide research parquet plus
evaluation reports.

This module is no longer part of the canonical DVC + Colab handoff + final
verdict path. Shared routing diagnostics live in ``src.routing_diagnostics``.

In the historical DVC research pipeline:
...
def compute_hybrid_routing(
    ...
    ml_adv_mask = np.array([is_adversarial_label(v) for v in ml_pred_binary], dtype=bool)
    unicode_lane, lane_reliable = compute_unicode_lane_mask(ml_df, unicode_types=unicode_types)
...
def write_margin_code_path_note(path, *, split: str) -> None:
    ...
    lines = [
        ...
        "",
        f"- Split: `{split}`",
        "- Historical experiment entrypoint: `python -m src.llm_classifier.llm_classifier --split test --research`",
        "- Historical hybrid routing entrypoint: `python -m src.research --split test`",
        "- Current canonical final report: `dvc repro final_verdict_report`",
        "",
        ...
        "3. `src.research.compute_hybrid_routing()` is the historical research-routing path. It applies ML fast-path routing, LLM abstain handling, and the configured margin policy.",
        ...
        "6. The canonical final-verdict report now consumes escalation-model and judged Colab handoff artifacts instead of this historical research parquet.",
        "",
...
def main():
    ...
    hybrid_df = compute_hybrid_routing(
        ...
        llm_generation_hint=(
            "legacy hosted LLM classifier stages were removed; use the canonical "
            "DVC + Colab handoff path in README.md for current runs"
        ),
